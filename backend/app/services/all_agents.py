...
class Agent1Anomaly:
    async def run(self, ctx: InvestigationContext) -> InvestigationContext:
        ...
        if result["disagreement_flag"]:
            logger.warning(
                f"[Agent1] Model disagreement — "
                f"XGB={result['xgb_score']:.3f} "
...
WATCHLIST = {
    "Indo-Gulf Trading Co",
    "Dubai Shell Corp",
    "Panama Holdings Ltd",
    "Hong Kong Layering LLC",
    "Cayman Islands Trust",
    "Crypto Gateway Ltd",
    "Singapore Offshore Co",
    "Al-Rashid Exchange",
...
class Agent2Evidence:
    async def run(self, ctx: InvestigationContext) -> InvestigationContext:
        ...
        if 0.85 * CTR_THRESHOLD <= amount < CTR_THRESHOLD:
            patterns.append("structuring_near_ctr")
            risk_boost += 0.25
        ...
        if hour in NIGHT_HOURS:
            patterns.append("night_hour_transaction")
            risk_boost += 0.10

        if channel in {"Wire Transfer", "Crypto"}:
            patterns.append("high_risk_channel")
            risk_boost += 0.15

        if amount > CTR_THRESHOLD:
            patterns.append("above_ctr_threshold")
            risk_boost += 0.20

        if old_bal > 0 and new_bal == 0:
            patterns.append("complete_balance_drain")
            risk_boost += 0.20

        watchlist_hits = self._check_watchlist(
            counterparty,
            location,
        )
        ...
        logger.info(
            f"[Agent2] patterns={patterns} "
            f"watchlist={len(watchlist_hits)}"
        )
...
class Agent3Network:
    ...
    async def run(
        ...
        nodes, edges, evidence = self._graph(
            ctx.transaction,
            history,
        )
        ...
        evidence["provisional_network_risk_score"] = (
            self._provisional_network_risk(evidence)
        )
    ...
    async def _history(
        ...
        for account in endpoints:
            found = await self.transaction_repository.get_by_account_roles(
                ...
                limit=self.HISTORY_LIMIT_PER_ENDPOINT,
                exclude_transaction_id=focal_id or None,
            )
    ...
    def _graph(
        ...
        records = [focal] + history
        edges = []

        for item in records:
            source = item.get("account_id")
        ...
        max_volume = max(
            (
                m["total_inflow"] + m["total_outflow"]
                for m in metrics.values()
            ),
            default=0.0,
        )
    ...
    def _metrics(
        ...
        for edge in edges:
            ...
            result[source]["out_degree"] += 1
            result[source]["total_outflow"] += amount

            result[destination]["in_degree"] += 1
            result[destination]["total_inflow"] += amount
    ...
    def _node(
        ...
        return {
            ...
            "suspicious_transaction_count": metric[
                "suspicious_transaction_count"
            ],
            "reciprocal_relationship_count": metric[
                "reciprocal_relationship_count"
            ],
            "total_inflow": round(
                metric["total_inflow"],
    ...
    def _evidence(
        ...
        return {
            ...
            "total_network_transaction_volume": round(
                sum(
                    e["amount"]
                    for e in edges
                ),
            ...
            "suspicious_transaction_count": suspicious,
            "suspicious_transaction_ratio": round(
                suspicious / count,
                4,
            )
            if count
            ...
            "reciprocal_relationship_count": sum(
                1
                for a, b in pairs
                if (
                    a < b
    ...
    @staticmethod
    def _provisional_network_risk(
        evidence,
    ):
        degree = (
            evidence["primary_network_degree"]
            / evidence["max_network_degree"]
            if evidence["max_network_degree"]
            else 0.0
        )

        centrality = (
            evidence["primary_pagerank"]
            / evidence["max_pagerank"]
            if evidence["max_pagerank"]
            else 0.0
        )
        ...
        return [
            {
                ...
                "reason": (
                    f"Elevated observed network behavior "
                    f"score {n['risk_score']:.2f} across "
                    f"{n['transaction_count']} transaction(s)"
                ),
                "auto_created": True,
            }
            for n in candidates[:MAX_SUBCASES]
        ]
    ...
    def _is_suspicious(
        ...
        is_fraud = transaction.get(
            "is_fraud"
        )

        normalized_is_fraud = (
            is_fraud.strip().lower()
            if isinstance(
                is_fraud,
                str,
            )
        ...
        return (
            is_fraud is True
            or (
                type(is_fraud) is int
                and is_fraud == 1
            )
            or normalized_is_fraud
            in {
                "1",
                "true",
            }
...
AGENT4_NETWORK_RISK_THRESHOLD = 0.65
...
class Agent4Regulatory:
    ...
    async def run(
        ...
        logger.info(
            f"[Agent4] Regulatory assessment for "
        ...
        fraud_probability = self._clamp(
            ctx.fraud_probability
        )
        ...
        # Agent 3 is safely consumed through InvestigationContext.
        # Partial or missing network data must not break Agent 4.
        network = (
            ctx.network
            if isinstance(
                ctx.network,
        ...
        # ---------------------------------------------------------------
        # Agent 3 behavioral network evidence
        # ---------------------------------------------------------------
        ...
        # Agent 3's provisional network risk is already bounded to [0, 1].
        # Agent 4 consumes it directly instead of deriving risk from
        # node count.
        network_risk_score = self._clamp(
            network_evidence.get(
                "provisional_network_risk_score",
                0.0,
            )
        )

        reciprocal_relationship_count = self._safe_int(
            network_evidence.get(
                "reciprocal_relationship_count",
                0,
            )
        ...
        suspicious_transaction_ratio = self._clamp(
            network_evidence.get(
        ...
        activity_concentration = self._clamp(
            network_evidence.get(
                "activity_concentration",
                0.0,
            )
        ...
        primary_network_degree = self._safe_int(
            network_evidence.get(
                "primary_network_degree",
                0,
            )
        ...
        cyclic_component_count = self._safe_int(
        ...
        largest_component_size = self._safe_int(
            strongly_connected_components.get(
        ...
        # Network existence remains available for compatibility and
        # typology support. Network size itself does not determine risk.
        has_network_signal = (
            node_count
            > AGENT4_NETWORK_SIGNAL_NODES
        )

        # Strong network risk is determined by Agent 3's behavioral
        # network-risk score rather than node count alone.
        has_strong_network_signal = (
            network_risk_score
            >= AGENT4_NETWORK_RISK_THRESHOLD
        )

        watchlist_hit = bool(
            ctx.watchlist_hit
            or ctx.watchlist_hits
        )
        ...
        for code, typology in FATF_TYPOLOGIES.items():
            ...
            confidence = min(
                ...
                + (
                    base_confidence
                    * AGENT4_TYPOLOGY_CONFIDENCE_SCALE
                ),
            )
            ...
            supporting_evidence.append(
                {
                    ...
                    "items": {
                        ...
                        # Actual Agent 3 behavioral evidence
                        "provisional_network_risk_score": (
                            network_risk_score
                        ),
                        "reciprocal_relationship_count": (
        ...
        if has_network_signal:
            # Agent 3 has already computed a bounded behavioral
            # network-risk score. Reuse it directly.
            network_signal = network_risk_score
            risk_components.append(
                network_signal
            )

        # Fraud contributes to overall investigation risk, but never
        # creates an STR recommendation by itself.
        risk_components.append(
            fraud_probability
        )
        ...
        # Explicitly bound the final result.
        ...
        if (
            risk_score
            >= AGENT4_RISK_CRITICAL_THRESHOLD
        ):
            overall_risk = "critical"

        elif (
            risk_score
            >= AGENT4_RISK_HIGH_THRESHOLD
        ):
            overall_risk = "high"

        elif (
            risk_score
            >= AGENT4_RISK_MEDIUM_THRESHOLD
        ):
            overall_risk = "medium"
        ...
        strong_evidence = (
            evidence_confidence
            >= AGENT4_STRONG_EVIDENCE_THRESHOLD
        )
        ...
        if (
            ...
            reportability_confidence = min(
                ...
                + (
                    0.10
                    * independent_regulatory_signals
                )
                + (
                    0.20
                    * evidence_confidence
            ...
            reportability_reason = (
                "Multiple independent regulatory indicators are "
                "supported by available investigation evidence."
            )
        ...
        if has_network_signal:
            rationale_parts.append(
                f"Network investigation contains "
                f"{node_count} nodes and "
                f"{sub_case_count} sub-case(s), "
                f"with behavioral network risk "
                f"{network_risk_score:.2f}."
            )
...
class Agent5Explanation:
    async def run(
        ...
        logger.info(
            f"[Agent5] Generating explanation for "
            f"{ctx.transaction_id}"
        )
    ...
    def _build_explanation(
        ...
        typos = ctx.regulatory.get(
            "fatf_typologies",
            [],
        )
        patterns = ctx.evidence.get(
            "patterns",
            [],
        )
        nodes = ctx.network.get(
            "node_count",
            0,
        )
        ...
        lines = [
            ...
            f"flagged with {level.upper()} risk "
            f"({prob:.1%})."
        ]
        ...
        if nodes > 2:
            lines.append(
                f"Network: {nodes} connected accounts, "
...
class Agent6Recommend:
    async def run(
        ...
        reg_basis = (
            f"PMLA 2002 {', '.join(pmla)} | "
            f"FATF "
            f"{typos[0]['code'] if typos else 'N/A'} | "
    ...
    def _action(
        ...
        if prob >= 0.65 or str_req:
            return "FILE_STR"

        if disagree and prob >= 0.4:
            return "ESCALATE"

        if prob >= 0.50 or network:
            return "ESCALATE"
    ...
    def _confidence(
        ...
        w = {
            "agent1_anomaly": 0.35,
            "agent2_evidence": 0.20,
            "agent3_network": 0.15,
            "agent4_regulatory": 0.20,
    ...
    def _reasoning(
        ...
        parts = [
            f"XGBoost="
            f"{ctx.anomaly_scores.get('xgb_score', 0):.2f}"
        ]
        ...
        if network:
            parts.append(
                f"{ctx.network.get('node_count', 0)}-account ring, "
                f"{len(ctx.sub_cases)} sub-case(s)"
            )
        ...
        if ctx.watchlist_hit:
            parts.append(
                ...
                + ", ".join(
                    h["entity"]
                    for h in ctx.watchlist_hits[:2]
                )
            )

        if ctx.disagreement_flag:
            parts.append(
                "Model disagreement — novel pattern"
            )
